...
class UrlUri{
    ...
    public static function setEstadoHeader($code = 200, $text = ''){
        ...
        $states = static::httpStates();
        //Me fijo que el codigo no sea un string 
        if ($code == '' OR ! is_numeric($code)){
            Error::general_error('Error Estado HTTP', 'El codigo de estado debe ser numerico');
	}
	...
	if (isset($states[$code]) AND $text == ''){
        ...
        if ($text == ''){
            Error::general_error('Error Estado HTTP', 'No status text available.  Please check your status code number or supply your own message text.');
	}
        //Cargo el protocolo
    ...
    public static function httpStates(){
        return array(
            100     => 'Continue',
